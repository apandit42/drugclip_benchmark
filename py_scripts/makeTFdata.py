...
def main(moltype, train_split, output_dir, data_df_path):
    ...
    run_batched_lmdb(df, output_dir,moltype= moltype)
...
def pocket_parser(paths_and_key, raid=6):
    protein_path, ligand_path,pocket_key = paths_and_key
    protein = read_pdb(protein_path)
...
def write_lmdb_pdb(data, lmdb_path):
    ...
    with env.begin(write=True) as txn:
        for d in data:
            txn.put(d['pocket_index'].encode(), pickle.dumps(d))
            num += 1
...
def construct_path_and_key(dfrow):
    uniprot_id = dfrow['uniprot_id']
    pdb_id = dfrow['pdb_id']
    base_path = "/mnt/goon/pdb_holo_structs/holo_prepped_structures/"
    base_path_out =f"{base_path}/{uniprot_id}/{pdb_id}.pdb"
    return base_path_out, base_path_out.replace('prot', 'lig'), f"{uniprot_id}_{pdb_id}"


def process_uniprot_chunk(args):
    i, df_batch, output_dir = args
    lmdb_path = os.path.join(output_dir, f"train_chunk_{i:03d}.lmdb")
    data  = [pocket_parser(construct_path_and_key(row)) for i,row in df_batch.iterrows()]
    write_lmdb_pdb(data, lmdb_path)
    return lmdb_path
...
def run_batched_lmdb(df, output_dir, batch_size=200, nprocs=384, moltype = 'lig'):
    ...
    with mp.Pool(processes=nprocs) as pool:
        if moltype == 'lig':
            ...
            results = pool.map(process_uniprot_chunk, indexed_batches)
